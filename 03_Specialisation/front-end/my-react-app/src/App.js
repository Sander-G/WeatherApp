...
useEffect(() => {
    ...
    fetch(`http://api.openweathermap.org/geo/1.0/direct?q=${city}&limit=5&appid=${process.env.REACT_APP_API_KEY}`)
        .then(res => res.json())
        .then(data => {
    setLocation({
...
useEffect(() => {
  fetch(`https://api.openweathermap.org/data/2.5/onecall?lat=${location.lat}&lon=${location.lon}&units=metric&appid=${process.env.REACT_APP_API_KEY}`)
        .then(res => res.json())
        .then(weatherData => {
            return setData({
  ...
  return (
    <>
      ...
      <div className="inputfield">
            ...
            <button id="button" onClick={searchLocation} >Search</button>
        </div>  
